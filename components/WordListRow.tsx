      ...
      <div className="flex min-h-[64px] items-center gap-1.5 px-2 py-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center gap-1">
            ...
            <button type="button" onClick={onToggleFavorite} aria-label={favorite ? '즐겨찾기 해제' : '즐겨찾기 저장'} className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-base ${favorite ? 'text-amber-500' : 'text-slate-300'}`}>{favorite ? '★' : '☆'}</button>
          </div>
        ...
        <div className="grid shrink-0 grid-cols-4 gap-0.5">
          <ActionButton label={`${word.word} 미국식 발음 듣기`} icon="🔊" onClick={() => speakAmericanEnglish(word.word)} />
          <ActionButton label={`${word.word} 말하기 연습`} icon="🗣" active={panel === 'practice'} onClick={() => setPanel((value) => value === 'practice' ? null : 'practice')} />
          <ActionButton label={`${word.word} 예문 보기`} icon="📖" active={panel === 'example'} onClick={() => setPanel((value) => value === 'example' ? null : 'example')} />
          <ActionButton label={`${word.word} 오답노트`} icon="📝" active={wrong} danger={wrong} onClick={onToggleWrong} />
        </div>
      </div>
      ...
      {panel === 'example' && (
        <div className="border-t border-emerald-100 bg-emerald-50/60 px-4 py-3">
          <div className="flex items-start justify-between gap-3">
            ...
            <button type="button" onClick={() => speakAmericanEnglish(word.example, 0.78)} className="min-h-10 shrink-0 rounded-xl bg-white px-3 text-xs font-extrabold text-leaf shadow-sm">🔊 듣기</button>
          </div>
...
function ActionButton({ label, icon, onClick, active = false, danger = false }: { label: string; icon: string; onClick: () => void; active?: boolean; danger?: boolean }) {
  return (
    <button
      type="button"
      aria-label={label}
      title={label}
      onClick={onClick}
      className={`flex h-10 w-9 items-center justify-center rounded-lg text-[17px] active:scale-90 ${danger ? 'bg-rose-100' : active ? 'bg-emerald-100' : 'bg-slate-50'}`}
    >
      {icon}
    </button>
  );
